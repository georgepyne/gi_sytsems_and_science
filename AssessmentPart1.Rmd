...
This study will compare a traditional GUI-based GIS (QGIS) and a code-based GIS (R) – by creating two similar maps and comparing each workflow. This study will provide: an outline of the data and methodological workflow used, the results of both GI Systems, a discussion on the benefits and drawbacks of each GI Systems and conclude the findings of this study.
...
__Table 1 - Indicators udes to create Neighbourhood Equity Scores (SPAR City of Toronto, 2014)__
...
State support | Higher education |               | Health food access | Hospitalisation               |                  |               | Green space        | Diabetes

Python’s pandas and geopandas packages were used to: clean, format and merge Toronto’s neighbourhood shapefile with the NES data. Both GI Systems then classified 5 Jenks quintiles based on the Toronto NES data. QGIS mapped these quintiles onto a basemap of the Toronto area by creating a level of transparency of the NES layer. Whereas R plotted the same NES quintiles on an interactive leaflet map. This map object was then assigned a polygon overlay with hover functionality – the Neighbourhood name and NES is presented when a mouse is hovered over each polygon.
...
Code-based GIS has a steeper learning curve, and it can be confusing as there are multiple ways of achieving the same objective (Harris, 2016). Brunsdon and Singleton (2015) argue this is a pragmatic problem of code-based GIS - it assumes users have adequate computational power and knowledge of programming.

Whilst QGIS (used in this study) is an open-source software, the methods used aren’t easily sharable. Documentation is advisable, otherwise the benefits of reproducibility and peer review are lost (Miller and Goodchild, 2015). GUI-based GIS is limited in its output formats; the interactive hover tool and zoom function of Figure 2.A is a distinct advantage of code-based GIS over GUI-based maps. GUI-GIS is traditionally more adept at cartographic conventions (see the legend, scale bar and north arrow; Figure 1). R however still provided a means of including a legend and interactive scale bar.
...
Brunsdon and Singleton (2015) argue that currently code and GUI-based GIS are generally interchangeable which will likely continue - however the traditional user base of GUI-based GIS will be affected as open-source code matures. This study generally reiterated this viewpoint, but critically it only provided two similar maps. Further research is suggested to compare the two systems using more advanced spatial analysis techniques.
